<template>
  ...
  v-if="show"
  >
    <form  @submit.prevent="updateReason" method="POST"
...
<script setup>
    ...
    const props = defineProps(['show','reason'])
    ...
    libelle: props.reason.libelle || "",
    description: props.reason.description || "",
    });
    const updateReason = () => {
        if (isSubmitting.value) return;
            isSubmitting.value = true;
        form.post(route('reasons.update', props.reason.id), {
            onSuccess: () => {
                form.reset();
    ...
    if (newreason) {
        form.code = props.reason.code || "",
        form.libelle = props.reason.libelle || "",
        form.description = props.reason.description || "";
    }
    },
    { immediate: true }
    )
