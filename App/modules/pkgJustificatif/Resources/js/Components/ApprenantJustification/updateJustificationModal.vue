  ...
  >
    ...
    >
      <div :class="['bg-white animate-popup m-4 p-6 rounded-lg shadow-xl w-3/4 overflow-y-auto max-h-[calc(100vh-2vh)] dark:bg-gray-800',animatingOut ? 'animate-popupOut' : 'animate-popupIn']">
        ...
        <p v-if="messageValidateStatError" class="text-red-600 font-semibold mb-4">
            {{ messageValidateStatError }}
        </p>
...
<script setup>
    ...
    const page = usePage();
    const messageValidateStatError = computed(() => page.props.flash?.messageErrorValideEnAttente);
    ...
    const updateJustification = () => {
        if (isSubmitting.value) return;
         ...
         form.post(route('Justificatifs.apprenant.update', props.justification.id), {
            forceFormData: true,
            _method: 'put',
            onSuccess: () => {
                if (!messageValidateStatError.value) {
    ...
    function closePopup() {
        page.props.flash.messageErrorValideEnAttente = null;
        form.clearErrors();
        animatingOut.value = true;
        setTimeout(() => {
        emit('close');
